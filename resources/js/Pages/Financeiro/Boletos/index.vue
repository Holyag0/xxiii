    ...
    <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
     <div class="grid grid-cols-4 gap-6">
            <div class="px-8 sm:col-span-2 lg:col-span-2">
              <input v-model="filtro.nome" type="text" placeholder="Pesquisar por aluno" class="block w-full py-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
            </div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase">Nº Transação</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="boleto in boletos.data" :key="boleto.id">
               
                <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                    {{ boleto.venda.aluno.nome }}
                </td> 
                
                <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                    {{ boleto.venda.aluno.cpf }}
                </td>
                ...
                <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                  <a target="_blank" v-if="boleto.status != 'canceled'" class="text-sm text-blue-600 hover:text-blue-400" :href="boleto.boleto">Visualizar</a>
                  <a target='_self' v-if="boleto.status != 'canceled'" class="pl-2 text-sm text-blue-600 hover:text-blue-400" :href="route('boleto.cancelar', boleto.transaction_id)">Cancelar</a>
                </td>
