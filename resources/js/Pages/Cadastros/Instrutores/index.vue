<template>
  ...
  <div class="flex flex-col">
    ...
    <title-and-button-cad :link="'instrutor.create'" :title="'Listagem Instrutores'" />
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase">Celular</th>
                <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase">Ações</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="instrutor in instrutores.data" :key="instrutor.id">
               ...
               <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                ...
                <td class="flex px-6 py-4 text-sm font-medium text-right-inline">
                  <Link v-if="getPermission('instrutor-show')" :href="route('instrutor.show', instrutor.id)" class="mr-2 text-sm text-blue-600 hover:text-blue-400">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                  </Link>
                  
                  <Link v-if="getPermission('instrutor-edit')" :href="route('instrutor.edit', instrutor.id)" class="text-sm text-blue-600 hover:text-blue-400">
  ...
  <Pagination :links="instrutores.links" class="mt-6"/>
</template>
  ...
  setup() {
    return {
       links:[
        ...
        { name: 'Listagem de Instrutores', current: true },
      ]
    }
  },
  props: {
