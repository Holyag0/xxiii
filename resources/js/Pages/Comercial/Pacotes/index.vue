...
<template>
  ...
  <div class="flex flex-col">
    ...
    <title-and-button-cad v-if="getPermission('pacote-create')" :link="'pacote.create'" :title="'Listagem Pacotes'" />
        ...
        <div class="overflow-hidden border-b border-gray-200 shadow sm:rounded-lg">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="pacote in pacotes.data" :key="pacote.id">
               ...
               <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                ...
                <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                    {{ pacote.nome }} | {{convertFloatMoney(valorPacote(pacote.servicos))}}
                </td>
             
                
                <td class="flex px-6 py-4 text-sm font-medium text-right-inline">
                  <Link v-if="getPermission('pacote-show')" :href="route('pacote.show', pacote.id)" class="mr-2 text-sm text-blue-600 hover:text-blue-400">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                  </Link>
                  
                  <Link v-if="getPermission('pacote-edit')" :href="route('pacote.edit', pacote.id)" class="text-sm text-blue-600 hover:text-blue-400">
  ...
  <Pagination :links="pacotes.links" class="mt-6"/>
</template>
...
export default {
  ...
  setup() {
    return {
       links:[
        { name: 'Menu Comercial', href: 'menuComercial', current: false },
        { name: 'Listagem de Pacotes', current: true },
      ]
    }
  },
  props: {
