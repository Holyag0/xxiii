...
<template>
  ...
  <div class="flex flex-col">
    ...
    <title-and-button-cad v-if="getPermission('servico-create')" :link="'servico.create'" :title="'Listagem Servicos'" />
              ...
              <tr v-for="servico in servicos.data" :key="servico.id">
               ...
               <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                ...
                <td class="flex px-6 py-4 text-sm font-medium text-right-inline">
                  <Link v-if="getPermission('servico-show')" :href="route('servico.show', servico.id)" class="mr-2 text-sm text-blue-600 hover:text-blue-400">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                  </Link>
                  
                  <Link v-if="getPermission('servico-edit')" :href="route('servico.edit', servico.id)" class="text-sm text-blue-600 hover:text-blue-400">
  ...
  <Pagination :links="servicos.links" class="mt-6"/>
</template>
  ...
  setup() {
    return {
       links:[
        ...
        { name: 'Listagem de Servicos', current: true },
      ]
    }
  },
  props: {
